...
CLASS lhc_travel DEFINITION INHERITING FROM cl_abap_behavior_handler
  ...
  PRIVATE SECTION.
    ...
    TYPES tt_travel_update TYPE TABLE FOR UPDATE ZTK_I_Travel_M.
...
**********************************************************************
  METHOD validate_customer.
    ...
    READ ENTITIES OF ZTK_I_Travel_M IN LOCAL MODE
    ENTITY travel
     FIELDS ( customer_id )
     WITH CORRESPONDING #(  keys )
    RESULT DATA(travels).

    DATA customers TYPE SORTED TABLE OF ZTK_customer WITH UNIQUE KEY customer_id.
    ...
    IF customers IS NOT INITIAL.
      ...
      SELECT FROM ZTK_customer FIELDS customer_id
    ...
    LOOP AT travels INTO DATA(travel).
      IF travel-customer_id IS INITIAL
        ...
        APPEND VALUE #(  travel_id = travel-travel_id
                         %msg      = NEW ZTK_cm_flight_messages(
                                                     customer_id = travel-customer_id
                                                     textid      = ZTK_cm_flight_messages=>customer_unkown
...
**********************************************************************
  ...
  METHOD validate_agency.
    ...
    DATA agencies TYPE SORTED TABLE OF ZTK_agency WITH UNIQUE KEY agency_id.
    ...
    IF  agencies IS NOT INITIAL.
      ...
      SELECT FROM ZTK_agency FIELDS agency_id
    ...
    LOOP AT travels INTO DATA(travel).
      IF travel-agency_id IS INITIAL
        ...
        APPEND VALUE #(  travel_id = travel-travel_id
                         %msg      = NEW ZTK_cm_flight_messages(
                                          textid    = ZTK_cm_flight_messages=>agency_unkown
  ...
  METHOD validate_dates.
    ...
    LOOP AT travels INTO DATA(travel).
      ...
      IF travel-end_date < travel-begin_date.  "end_date before begin_date
        ...
        APPEND VALUE #( %key     = travel-%key
                        %msg = NEW ZTK_cm_flight_messages(
                                         textid     = ZTK_cm_flight_messages=>begin_date_bef_end_date
                                         severity   = if_abap_behv_message=>severity-error
      ...
      ELSEIF travel-begin_date < cl_abap_context_info=>get_system_date( ).  "begin_date must be in the future
        ...
        APPEND VALUE #( %key = travel-%key
                        %msg = NEW ZTK_cm_flight_messages(
                               textid = ZTK_cm_flight_messages=>begin_date_on_or_bef_sysdate
                               severity = if_abap_behv_message=>severity-error )
                        %element-begin_date = if_abap_behv=>mk-on
                        %element-end_date   = if_abap_behv=>mk-on
                      ) TO reported-travel.
    ...
    LOOP AT travels INTO DATA(travel).
      CASE travel-overall_status.
        ...
        WHEN OTHERS.
          ...
          APPEND VALUE #( %key = travel-%key
                          %msg = NEW ZTK_cm_flight_messages(
                               textid = ZTK_cm_flight_messages=>status_invalid
                               severity = if_abap_behv_message=>severity-error
...
**********************************************************************
  METHOD copyTravel.
    ...
    DATA:
      travels       TYPE TABLE FOR CREATE ZTK_I_Travel_M\\travel,
      bookings_cba  TYPE TABLE FOR CREATE ZTK_I_Travel_M\\travel\_booking,
      booksuppl_cba TYPE TABLE FOR CREATE ZTK_I_Travel_M\\booking\_booksupplement.
    ...
    MODIFY ENTITIES OF ZTK_I_Travel_M IN LOCAL MODE
      ENTITY travel
...
*
  ...
  METHOD earlynumbering_create.
    ...
    DATA:
      entity        TYPE STRUCTURE FOR CREATE ZTK_I_Travel_M,
      travel_id_max TYPE ZTK_travel_id.
      ...
      WHEN '1'.
        ...
        LOOP AT entities_wo_travelid INTO entity.
          APPEND VALUE #(
                ...
                %key      = entity-%key
                %msg      = NEW ZTK_cm_flight_messages(
                                textid = ZTK_cm_flight_messages=>number_range_depleted
      ...
      WHEN '2' OR '3'.
        ...
        LOOP AT entities_wo_travelid INTO entity.
          APPEND VALUE #(
                ...
                %key      = entity-%key
                %msg      = NEW ZTK_cm_flight_messages(
                                textid = ZTK_cm_flight_messages=>not_sufficient_numbers
                                severity = if_abap_behv_message=>severity-error )
            ) TO reported-travel.
          APPEND VALUE #(
                %cid        = entity-%cid
  ...
  METHOD earlynumbering_cba_booking.
    DATA: max_booking_id TYPE ZTK_booking_id.
    ...
    LOOP AT entities ASSIGNING FIELD-SYMBOL(<travel_group>) GROUP BY <travel_group>-travel_id.
      ...
      max_booking_id = REDUCE #( INIT max = CONV ZTK_booking_id( '0' )
                                 FOR  booking IN bookings USING KEY entity WHERE ( source-travel_id  = <travel_group>-travel_id )
                                 NEXT max = COND ZTK_booking_id( WHEN booking-target-booking_id > max
      ...
      max_booking_id = REDUCE #( INIT max = max_booking_id
                                 ...
                                 NEXT max = COND ZTK_booking_id( WHEN   target-booking_id > max
...
********************************************************************************
    ...
    DATA travel_log        TYPE STANDARD TABLE OF ZTK_log_travel.
    DATA travel_log_create TYPE STANDARD TABLE OF ZTK_log_travel.
    DATA travel_log_update TYPE STANDARD TABLE OF ZTK_log_travel.
    ...
    IF create-travel IS NOT INITIAL.
      ...
      " Inserts rows specified in lt_travel_log_c into the DB table ZTK_log_travel
      INSERT ZTK_log_travel FROM TABLE @travel_log_create.
    ...
    IF update-travel IS NOT INITIAL.
      ...
      " Inserts rows specified in lt_travel_log_u into the DB table ZTK_log_travel
      INSERT ZTK_log_travel FROM TABLE @travel_log_update.
    ...
    IF delete-travel IS NOT INITIAL.
      ...
      " Inserts rows specified in lt_travel_log into the DB table ZTK_log_travel
      INSERT ZTK_log_travel FROM TABLE @travel_log.
...
*
********************************************************************************
    DATA booksuppls_db TYPE STANDARD TABLE OF ZTK_booksuppl_m.

    " (1) Get instance data of all instances that have been created
    IF create-booksuppl IS NOT INITIAL.
      booksuppls_db = CORRESPONDING #( create-booksuppl ).

      CALL FUNCTION 'ZTK_FLIGHT_BOOKSUPPL_C' EXPORTING values = booksuppls_db .
    ...
    IF booksuppls_db IS NOT INITIAL.
      ...
      SELECT * FROM ZTK_booksuppl_m FOR ALL ENTRIES IN @booksuppls_db
      ...
      CALL FUNCTION 'ZTK_FLIGHT_BOOKSUPPL_U' EXPORTING values = booksuppls_db .
    ...
    IF delete-booksuppl IS NOT INITIAL.
      ...
      CALL FUNCTION 'ZTK_FLIGHT_BOOKSUPPL_D' EXPORTING values = booksuppls_db .
