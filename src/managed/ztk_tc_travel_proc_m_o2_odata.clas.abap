"!@testing SRVB:ZTK_UI_TRAVEL_PROC_M_O2
...
CLASS ZTK_tc_travel_proc_m_o2_odata DEFINITION
  FOR TESTING
  RISK LEVEL HARMLESS
  DURATION SHORT
  PUBLIC
...
CLASS ZTK_tc_travel_proc_m_o2_odata  IMPLEMENTATION.

  METHOD class_setup.

    mo_client_proxy = create_local_client_proxy( VALUE #( service_id      = 'ZTK_UI_TRAVEL_PROC_M_O2'
                                                          service_version = '0001' )  ).

    " Create the stubs/doubles for the underlying CDS entities
    cds_test_environment = cl_cds_test_environment=>create_for_multiple_cds(
                      i_for_entities = VALUE #(
                        ( i_for_entity = 'ZTK_C_TRAVEL_PROCESSOR_M'    i_select_base_dependencies = abap_true )
                        ( i_for_entity = 'ZTK_C_BOOKING_PROCESSOR_M'   i_select_base_dependencies = abap_true )
                        ( i_for_entity = 'ZTK_C_BOOKSUPPL_PROCESSOR_M' i_select_base_dependencies = abap_true ) ) ).
...
*        ( 'ZTK_AGENCY' )      "already mocked by cds_test_environment above
*        ( 'ZTK_CUSTOMER' )    "already mocked by cds_test_environment above
*        ( 'ZTK_CARRIER' )     "already mocked by cds_test_environment above
        ( 'ZTK_FLIGHT' )
        ( 'ZTK_SUPPLEMENT' )
  ...
  METHOD create_travel.
    ...
    DATA(ls_business_data) = VALUE ZTK_c_travel_processor_m(
    ...
    DATA ls_response_data TYPE ZTK_c_travel_processor_m.
    ...
    READ ENTITIES OF ZTK_c_travel_processor_m
    ...
    SELECT single @abap_true FROM ZTK_i_travel_m INTO @DATA(lv_travel). "#EC CI_NOWHERE
