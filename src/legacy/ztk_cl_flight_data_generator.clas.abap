CLASS ZTK_cl_flight_data_generator DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
  ...
  PRIVATE SECTION.
    CLASS-METHODS:
      ...
      "!  <br/>
      "! Propagation to ZTK_cl_flight_legacy=>calculate_flight_price.<br/>
      ...
      calculate_flight_price
        ...
        RETURNING
          VALUE(rv_price)           TYPE ZTK_flight_price.

ENDCLASS.



CLASS ZTK_cl_flight_data_generator IMPLEMENTATION.


  METHOD if_oo_adt_classrun~main.
    out->write( 'Starting Data Generation' ) ##NO_TEXT.

    out->write( 'Generate Data: Airport      ZTK_AIRPORT' ) ##NO_TEXT.
    lcl_airport_data_generator=>lif_data_generator~create( out ).

    out->write( 'Generate Data: Carrier      ZTK_CARRIER' ) ##NO_TEXT.
    lcl_carrier_data_generator=>lif_data_generator~create( out ).
    out->write( 'Generate Data: Connection   ZTK_CONNECTION' ) ##NO_TEXT.
    lcl_connection_data_generator=>lif_data_generator~create( out ).
    out->write( 'Generate Data: Flight       ZTK_FLIGHT' ) ##NO_TEXT.
    lcl_flight_data_generator=>lif_data_generator~create( out ).


    out->write( 'Generate Data: Agency       ZTK_AGENCY' ) ##NO_TEXT.
    lcl_agency_data_generator=>lif_data_generator~create( out ).

    out->write( 'Generate Data: Customer      ZTK_CUSTOMER' ) ##NO_TEXT.
    lcl_customer_data_generator=>lif_data_generator~create( out ).

    out->write( 'Generate Data: Supplement      ZTK_SUPPLEMENT' ) ##NO_TEXT.
    lcl_supplement_data_generator=>lif_data_generator~create( out ).

    out->write( 'Generate Data: Travel      ZTK_TRAVEL' ) ##NO_TEXT.
    out->write( 'Generate Data: Booking      ZTK_BOOKING' ) ##NO_TEXT.
    out->write( 'Generate Data: Booking Supplement      ZTK_BOOK_SUPPL' ) ##NO_TEXT.
    ...
    DATA lo_badi TYPE REF TO ZTK_data_generation_badi.
    GET BADI lo_badi.
  ...
  METHOD calculate_flight_price.
    rv_price = ZTK_cl_flight_legacy=>calculate_flight_price(
