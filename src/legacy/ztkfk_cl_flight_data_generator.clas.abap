CLASS ZTKFK_cl_flight_data_generator DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
  ...
  PRIVATE SECTION.
    CLASS-METHODS:
      ...
      "!  <br/>
      "! Propagation to ZTKFK_cl_flight_legacy=>calculate_flight_price.<br/>
      ...
      calculate_flight_price
        ...
        RETURNING
          VALUE(rv_price)           TYPE ZTKFK_flight_price.

ENDCLASS.



CLASS ZTKFK_cl_flight_data_generator IMPLEMENTATION.


  METHOD if_oo_adt_classrun~main.
    out->write( 'Starting Data Generation' ) ##NO_TEXT.

    out->write( 'Generate Data: Airport      ZTKFK_AIRPORT' ) ##NO_TEXT.
    lcl_airport_data_generator=>lif_data_generator~create( out ).

    out->write( 'Generate Data: Carrier      ZTKFK_CARRIER' ) ##NO_TEXT.
    lcl_carrier_data_generator=>lif_data_generator~create( out ).
    out->write( 'Generate Data: Connection   ZTKFK_CONNECTION' ) ##NO_TEXT.
    lcl_connection_data_generator=>lif_data_generator~create( out ).
    out->write( 'Generate Data: Flight       ZTKFK_FLIGHT' ) ##NO_TEXT.
    lcl_flight_data_generator=>lif_data_generator~create( out ).


    out->write( 'Generate Data: Agency       ZTKFK_AGENCY' ) ##NO_TEXT.
    lcl_agency_data_generator=>lif_data_generator~create( out ).

    out->write( 'Generate Data: Customer      ZTKFK_CUSTOMER' ) ##NO_TEXT.
    lcl_customer_data_generator=>lif_data_generator~create( out ).

    out->write( 'Generate Data: Supplement      ZTKFK_SUPPLEMENT' ) ##NO_TEXT.
    lcl_supplement_data_generator=>lif_data_generator~create( out ).

    out->write( 'Generate Data: Travel      ZTKFK_TRAVEL' ) ##NO_TEXT.
    out->write( 'Generate Data: Booking      ZTKFK_BOOKING' ) ##NO_TEXT.
    out->write( 'Generate Data: Booking Supplement      ZTKFK_BOOK_SUPPL' ) ##NO_TEXT.
    ...
    DATA lo_badi TYPE REF TO ZTKFK_data_generation_badi.
    GET BADI lo_badi.
  ...
  METHOD calculate_flight_price.
    rv_price = ZTKFK_cl_flight_legacy=>calculate_flight_price(
