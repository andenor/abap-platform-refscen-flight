CLASS lhc_booking DEFINITION
  ...
  PRIVATE SECTION.
    ...
    TYPES tt_bookingsupplement_failed   TYPE TABLE FOR FAILED   ZTK_i_bookingsupplement_u.
    TYPES tt_bookingsupplement_reported TYPE TABLE FOR REPORTED ZTK_i_bookingsupplement_u.
...
**********************************************************************
  METHOD update.
    DATA: messages TYPE ZTK_t_message,
          ...
          bookingx TYPE ZTK_s_booking_inx.
    ...
    LOOP AT entities ASSIGNING FIELD-SYMBOL(<booking>).
      ...
      bookingx-action_code = ZTK_if_flight_legacy=>action_code-update.

      CALL FUNCTION 'ZTK_FLIGHT_TRAVEL_UPDATE'
        EXPORTING
          is_travel   = VALUE ZTK_s_travel_in( travel_id = <booking>-travelid )
          is_travelx  = VALUE ZTK_s_travel_inx( travel_id = <booking>-travelid )
          it_booking  = VALUE ZTK_t_booking_in( ( CORRESPONDING #( booking ) ) )
          it_bookingx = VALUE ZTK_t_booking_inx( ( bookingx ) )
        IMPORTING
    ...
    LOOP AT keys ASSIGNING FIELD-SYMBOL(<booking>).
      ...
      CALL FUNCTION 'ZTK_FLIGHT_TRAVEL_UPDATE'
        EXPORTING
          is_travel   = VALUE ZTK_s_travel_in( travel_id = <booking>-travelid )
          is_travelx  = VALUE ZTK_s_travel_inx( travel_id = <booking>-travelid )
          it_booking  = VALUE ZTK_t_booking_in( ( booking_id = <booking>-bookingid ) )
          it_bookingx = VALUE ZTK_t_booking_inx( ( booking_id  = <booking>-bookingid
...
**********************************************************************
  METHOD read.
    DATA: travel_out   TYPE ZTK_travel,
          bookings_out TYPE ZTK_t_booking,
          messages     TYPE ZTK_t_message.
...
***********************************************************************
  METHOD rba_Booksupplement.
    DATA: bookingsupplements_out TYPE ZTK_t_booking_supplement,
          messages               TYPE ZTK_t_message.
...
***********************************************************************
  METHOD cba_Booksupplement.
    DATA: message                   TYPE LINE OF ZTK_t_message,
          messages                  TYPE ZTK_t_message,
          booksupplements_old       TYPE ZTK_t_booking_supplement,
          booksupplement            TYPE ZTK_book_suppl,
          last_bookingsupplement_id TYPE ZTK_booking_supplement_id.
    ...
    LOOP AT entities_cba ASSIGNING FIELD-SYMBOL(<booking>).
      ...
      ELSE.
        ...
        LOOP AT <booking>-%target ASSIGNING FIELD-SYMBOL(<supplement>).
          ...
          CALL FUNCTION 'ZTK_FLIGHT_TRAVEL_UPDATE'
            EXPORTING
              is_travel              = VALUE ZTK_s_travel_in( travel_id = parent_key-travelid )
              is_travelx             = VALUE ZTK_s_travel_inx( travel_id = parent_key-travelid )
              it_booking_supplement  = VALUE ZTK_t_booking_supplement_in( ( CORRESPONDING #( booksupplement ) ) )
              it_booking_supplementx = VALUE ZTK_t_booking_supplement_inx( ( VALUE #(
