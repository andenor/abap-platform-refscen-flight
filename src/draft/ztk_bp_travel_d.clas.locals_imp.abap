...
CLASS lhc_travel IMPLEMENTATION.
  ...
  METHOD acceptTravel.
    ...
    MODIFY ENTITIES OF ZTK_I_Travel_D IN LOCAL MODE
      ENTITY Travel
    ...
    READ ENTITIES OF ZTK_I_Travel_D IN LOCAL MODE
      ENTITY Travel
        ALL FIELDS WITH
        CORRESPONDING #( keys )
      RESULT DATA(travels).
  ...
  METHOD deductDiscount.
    DATA travels_for_update TYPE TABLE FOR UPDATE ZTK_I_Travel_D.
    ...
    LOOP AT keys_with_valid_discount ASSIGNING FIELD-SYMBOL(<key_with_valid_discount>) WHERE %param-discount_percent IS INITIAL
      ...
      APPEND VALUE #( %tky                = <key_with_valid_discount>-%tky
                      %msg                = NEW ZTK_cm_flight_messages(
                             textid = ZTK_cm_flight_messages=>discount_invalid
  ...
  METHOD setTravelNumber.
    ...
    "Get max travelID
    SELECT SINGLE FROM ZTK_a_travel_d FIELDS MAX( travel_id ) INTO @DATA(max_travelid).
    ...
    LOOP AT travels INTO DATA(travel).
      ...
      IF travel-CustomerID IS  INITIAL.
        ...
        APPEND VALUE #( %tky                = travel-%tky
                        %state_area         = 'VALIDATE_CUSTOMER'
                        %msg                = NEW ZTK_cm_flight_messages(
                                                                textid   = ZTK_cm_flight_messages=>enter_customer_id
                                                                severity = if_abap_behv_message=>severity-error )
  ...
  METHOD validateAgency.
    ...
    IF  agencies IS NOT INITIAL.
      " Check if customer ID exists
      SELECT FROM ZTK_agency FIELDS agency_id, country_code
    ...
    LOOP AT travels INTO DATA(travel).
      ...
      IF travel-AgencyID IS  INITIAL.
        ...
        APPEND VALUE #( %tky                = travel-%tky
                        %state_area         = 'VALIDATE_AGENCY'
                        %msg                = NEW ZTK_cm_flight_messages(
                                                          textid   = ZTK_cm_flight_messages=>enter_agency_id
                                                          severity = if_abap_behv_message=>severity-error )
      ...
      ELSE.
        ...
        IF modification_granted = abap_false.
          ...
          APPEND VALUE #(  %tky               = travel-%tky
                           ...
                           %msg               = NEW ZTK_cm_flight_messages(
                                                                  agency_id = travel-agencyid
                                                                  textid    = ZTK_cm_flight_messages=>not_authorized_for_agencyid
                                                                  severity  = if_abap_behv_message=>severity-error )
                           %element-AgencyID  = if_abap_behv=>mk-on
                        ) TO reported-travel.
        ENDIF.
    ...
    LOOP AT travels INTO DATA(travel).
      ...
      IF travel-BeginDate IS INITIAL.
        ...
        APPEND VALUE #( %tky               = travel-%tky
                        %state_area        = 'VALIDATE_DATES'
                         %msg              = NEW ZTK_cm_flight_messages(
                                                                textid   = ZTK_cm_flight_messages=>enter_begin_date
                                                                severity = if_abap_behv_message=>severity-error )
      ...
      IF travel-EndDate IS INITIAL.
        ...
        APPEND VALUE #( %tky               = travel-%tky
                        %state_area        = 'VALIDATE_DATES'
                         %msg                = NEW ZTK_cm_flight_messages(
                                                                textid   = ZTK_cm_flight_messages=>enter_end_date
                                                                severity = if_abap_behv_message=>severity-error )
  ...
  METHOD get_global_authorizations.
    IF requested_authorizations-%create EQ if_abap_behv=>mk-on.
      ...
      ELSE.
        ...
        APPEND VALUE #( %msg    = NEW ZTK_cm_flight_messages(
                                       textid   = ZTK_cm_flight_messages=>not_authorized
                                       severity = if_abap_behv_message=>severity-error )
...
.
  ...
  METHOD is_create_granted.
    ...
    IF country_code IS SUPPLIED.
      AUTHORITY-CHECK OBJECT 'ZTK_TRVL'
        ID 'ZTK_CNTRY' FIELD country_code
        ID 'ACTVT'      FIELD '01'.
...
*      ENDCASE.
  ...
  METHOD get_instance_authorizations.
    ...
    SELECT  FROM ZTK_a_travel_d AS travel  INNER JOIN ZTK_agency AS agency
