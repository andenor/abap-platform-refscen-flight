...
CLASS lhc_booking IMPLEMENTATION.
  ...
  METHOD setBookingNumber.
    DATA max_bookingid TYPE ZTK_booking_id.
    DATA bookings_update TYPE TABLE FOR UPDATE ZTK_I_Travel_D\\Booking.
    ...
    LOOP AT bookings ASSIGNING FIELD-SYMBOL(<booking>).
      ...
      IF <booking>-AirlineID IS INITIAL.
        ...
        APPEND VALUE #( %tky                = <booking>-%tky
                        %state_area         = 'VALIDATE_CONNECTION'
                         %msg                = NEW ZTK_cm_flight_messages(
                                                                textid = ZTK_cm_flight_messages=>ENTER_AIRLINE_ID
                                                                severity = if_abap_behv_message=>severity-error )
                        %path              = VALUE #( travel-%tky = travel_booking_links[ source-%tky = <booking>-%tky ]-target-%tky )
      ...
      IF <booking>-ConnectionID IS INITIAL.
        ...
        APPEND VALUE #( %tky                = <booking>-%tky
                        %state_area         = 'VALIDATE_CONNECTION'
                        %msg                = NEW ZTK_cm_flight_messages(
                                                                textid = ZTK_cm_flight_messages=>ENTER_CONNECTION_ID
                                                                severity = if_abap_behv_message=>severity-error )
                        %path               = VALUE #( travel-%tky = travel_booking_links[ source-%tky = <booking>-%tky ]-target-%tky )
      ...
      IF <booking>-FlightDate IS INITIAL.
        ...
        APPEND VALUE #( %tky                = <booking>-%tky
                        %state_area         = 'VALIDATE_CONNECTION'
                        %msg                = NEW ZTK_cm_flight_messages(
                                                                textid = ZTK_cm_flight_messages=>ENTER_FLIGHT_DATE
                                                                severity = if_abap_behv_message=>severity-error )
                        %path               = VALUE #( travel-%tky = travel_booking_links[ source-%tky = <booking>-%tky ]-target-%tky )
      ...
      IF <booking>-AirlineID IS NOT INITIAL AND
        ...
        SELECT SINGLE Carrier_ID, Connection_ID, Flight_Date   FROM ZTK_flight  WHERE  carrier_id    = @<booking>-AirlineID
        ...
        IF sy-subrc <> 0.
          ...
          APPEND VALUE #( %tky                 = <booking>-%tky
                          %state_area          = 'VALIDATE_CONNECTION'
                          %msg                 = NEW ZTK_cm_flight_messages(
                                                                textid      = ZTK_cm_flight_messages=>NO_FLIGHT_EXISTS
